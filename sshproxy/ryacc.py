...
def initialize_vars():
    ...
    ctx.Productions  = [None]  # A list of all of the productions.  The first
                           # entry is always reserved for the purpose of
                           # building an augmented grammar
                        
    ctx.Prodnames    = { }     # A dictionary mapping the names of nonterminals to a list of all
                           # productions of that nonterminal.
                        
    ctx.Prodmap      = { }     # A dictionary that is only used to detect duplicate
                           # productions.

    ctx.Terminals    = { }     # A dictionary mapping the names of terminal symbols to a
                           # list of the rules where they are used.

    ctx.Nonterminals = { }     # A dictionary mapping names of nonterminals to a list
                           # of rule numbers where they are used.

    ctx.First        = { }     # A dictionary of precomputed FIRST(x) symbols
    
    ctx.Follow       = { }     # A dictionary of precomputed FOLLOW(x) symbols

    ctx.Precedence   = { }     # Precedence rules for each terminal. Contains tuples of the
                           # form ('right',level) or ('nonassoc', level) or ('left',level)

    ctx.LRitems      = [ ]     # A list of all LR items for the grammar.  These are the
                           # productions with the "dot" like E -> E . PLUS E

    ctx.Errorfunc    = None    # User defined error handler

    ctx.Signature    = md5.new()   # Digital signature of the grammar rules, precedence
                               # and other information.  Used to determined when a
                               # parsing table needs to be regenerated.

    ctx.Requires     = { }     # Requires list

    # LALR(1) Initialization
    ctx.Prodempty    = { }     # A dictionary of all productions that have an empty rule
                           # of the form P : <empty>

    ctx.TReductions  = { }     # A dictionary of precomputer reductions from
                           # nonterminals to terminals

    ctx.NTReductions = { }     # A dictionary of precomputed reductions from
                           # nonterminals to nonterminals

    ctx.GotoSetNum   = { }     # A dictionary that remembers goto sets based on
                           # the state number and symbol

    ctx.Canonical    = { }     # A list of LR item sets. A LR item set is a list of LR
...
class Production:
    ...
    def lr_item(self,n):
        ...
        try:
            p.lrafter = ctx.Prodnames[p.prod[n+1]]
...
def add_production(f,file,line,prodname,syms):
    ...
    if ctx.Terminals.has_key(prodname):
    ...
    p.line = line
    p.func = f
    p.number = len(ctx.Productions)

            
    ctx.Productions.append(p)
    ctx.Prodmap[map] = p
    if not ctx.Nonterminals.has_key(prodname):
        ctx.Nonterminals[prodname] = [ ]
    
    # Add all terminals to ctx.Terminals
    i = 0
    while i < len(p.prod):
        t = p.prod[i]
        if t == '%prec':
            ...
            prec = ctx.Precedence.get(precname,None)
            if not prec:
        ...
        if ctx.Terminals.has_key(t):
            ctx.Terminals[t].append(p.number)
            # Is a terminal.  We'll assign a precedence to p based on this
            if not hasattr(p,"prec"):
                p.prec = ctx.Precedence.get(t,('right',0))
        else:
            if not ctx.Nonterminals.has_key(t):
                ctx.Nonterminals[t] = [ ]
            ctx.Nonterminals[t].append(p.number)
        i += 1
    ...
    try:
        ctx.Prodnames[p.name].append(p)
    except KeyError:
        ctx.Prodnames[p.name] = [ p ]
    return 0
...
def compute_reachable():
    ...
    '''
    Reachable = { }
    for s in ctx.Terminals.keys() + ctx.Nonterminals.keys():
        Reachable[s] = 0

    mark_reachable_from( ctx.Productions[0].prod[0], Reachable )

    for s in ctx.Nonterminals.keys():
        if not Reachable[s]:
...
def compute_terminates():
    ...
    # Initialize to false:
    for n in ctx.Nonterminals.keys():
        Terminates[n] = 0

    # Then propagate termination until no change:
    while 1:
        some_change = 0
        for (n,pl) in ctx.Prodnames.items():
    ...
    for (s,terminates) in Terminates.items():
        if not terminates:
            if not ctx.Prodnames.has_key(s) and not ctx.Terminals.has_key(s) and s != 'error':
...
def verify_productions(cycle_check=1):
    ...
    for s,v in ctx.Terminals.items():
    ...
    if yaccdebug:
        _vf.write("\nGrammar\n\n")
        for i in range(1,len(ctx.Productions)):
            _vf.write("Rule %-5d %s\n" % (i, ctx.Productions[i]))
        
    unused_prod = 0
    # Verify the use of all productions
    for s,v in ctx.Nonterminals.items():
        if not v:
            p = ctx.Prodnames[s][0]
    ...
    if yaccdebug:
        ...
        ks = ctx.Terminals.keys()
        ks.sort()
        for k in ks:
            _vf.write("%-20s : %s\n" % (k, " ".join([str(s) for s in ctx.Terminals[k]])))
        _vf.write("\nNonterminals, with rules where they appear\n\n")
        ks = ctx.Nonterminals.keys()
        ks.sort()
        for k in ks:
            _vf.write("%-20s : %s\n" % (k, " ".join([str(s) for s in ctx.Nonterminals[k]])))
...
def build_lritems():
    for p in ctx.Productions:
        ...
        while 1:
            ...
            lastlri.lr_next = lri
            if not lri: break
            lri.lr_num = len(ctx.LRitems)
            ctx.LRitems.append(lri)
            lastlri = lri
            i += 1
    ...
    for p in plist:
        ...
        try:
            ...
            if prec != 'left' and prec != 'right' and prec != 'nonassoc':
                ...
                return -1
            for t in terms:
                if ctx.Precedence.has_key(t):
                    sys.stderr.write("yacc: Precedence already specified for terminal '%s'\n" % t)
                    error += 1
                    continue
                ctx.Precedence[t] = (prec,plevel)
        except:
...
def augment_grammar(start=None):
    if not start:
        start = ctx.Productions[1].name
    ctx.Productions[0] = Production(name="S'",prod=[start],number=0,len=1,prec=('right',0),func=None)
    ctx.Productions[0].usyms = [ start ]
    ctx.Nonterminals[start].append(0)
...
def first(beta):
    ...
    # We are computing ctx.First(x1,x2,x3,...,xn)
    result = [ ]
    for x in beta:
        x_produces_empty = 0

        # Add all the non-<empty> symbols of ctx.First[x] to the result.
        for f in ctx.First[x]:
            if f == '<empty>':
                x_produces_empty = 1
            else:
...
def compute_follow(start=None):
    ...
    for k in ctx.Nonterminals.keys():
        ctx.Follow[k] = [ ]
    ...
    while 1:
        didadd = 0
        for p in ctx.Productions[1:]:
            # Here is the production set
            for i in range(len(p.prod)):
                B = p.prod[i]
                if ctx.Nonterminals.has_key(B):
                    # Okay. We got a non-terminal in a production
                    fst = first(p.prod[i+1:])
                    hasempty = 0
                    for f in fst:
                        if f != '<empty>' and f not in ctx.Follow[B]:
                            ctx.Follow[B].append(f)
                            didadd = 1
                        if f == '<empty>':
                            hasempty = 1
                    if hasempty or i == (len(p.prod)-1):
                        # Add elements of follow(a) to follow(b)
                        for f in ctx.Follow[p.name]:
                            if f not in ctx.Follow[B]:
                                ctx.Follow[B].append(f)
                                didadd = 1
        if not didadd: break
    ...
    if 0 and yaccdebug:
        _vf.write('\nFollow:\n')
        for k in ctx.Nonterminals.keys():
            _vf.write("%-20s : %s\n" % (k, " ".join([str(s) for s in ctx.Follow[k]])))
...
def compute_first1():
    ...
    while 1:
        some_change = 0
        for n in ctx.Nonterminals.keys():
            for p in ctx.Prodnames[n]:
                for f in first(p.prod):
                    if f not in ctx.First[n]:
                        ctx.First[n].append( f )
                        some_change = 1
        if not some_change:
            break
    ...
    if 0 and yaccdebug:
        ...
        for k in ctx.Nonterminals.keys():
            _vf.write("%-20s : %s\n" %
                (k, " ".join([str(s) for s in ctx.First[k]])))
...
def lr0_goto_setnumber(I_setnumber, x):
    ...
    if ctx.GotoSetNum.has_key((I_setnumber, x)):
        setnumber = ctx.GotoSetNum[(I_setnumber, x)]
    else:
        gset = lr0_goto(ctx.Canonical[I_setnumber], x)
        if not gset:
            return -1
        else:
            gsetlen = len(gset)            
...
def lr0_items():
    ...
    C = [ lr0_closure([ctx.Productions[0].lr_next]) ]
    i = 0
    for I in C:
        _lr0_cidhash[id(I)] = i
        i += 1
                        ...
                        for a in ctx.Follow[p.name]:
                            ...
                            if r is not None:
                                ...
                                if r > 0:
                                    ...
                                    sprec,slevel = ctx.Productions[actionp[st,a].number].prec                                    
                                    rprec,rlevel = ctx.Precedence.get(a,('right',0))
                                    ...
                                    oldp = ctx.Productions[-r]
                                    pp = ctx.Productions[p.number]
                        ...
                        if j >= 0:
                            ...
                            if r is not None:
                                ...
                                if r > 0:
                                    ...
                                    #   -  otherwise we shift
                                    rprec,rlevel = ctx.Productions[actionp[st,a].number].prec
                                    sprec,slevel = ctx.Precedence.get(a,('right',0))
                                    if (slevel > rlevel) or ((slevel == rlevel) and (rprec != 'left')):
...
def lr1_closure(I, setnumber = 0):
    ...
    while didadd:
        ...
        for j in J:
            ...
            if jlr_index < len(jprod) - 1 and ctx.Nonterminals.has_key(jprod[jlr_index+1]):
                first_syms = []
...
def ReduceNonterminals():
    ...
    for nt in ctx.Nonterminals.keys():
        ctx.TReductions[nt] = []
        ctx.NTReductions[nt] = []
        
    for nt in ctx.Nonterminals.keys():
        terms = ReduceToTerminals(nt)
        ctx.TReductions[nt].extend(terms)
        if not ctx.NTReductions.has_key(nt):
...
def ReduceToTerminals(nt,cyclemap=None):
    ...
    for p in ctx.Prodnames[nt]:
        if len(p.prod) > 0:
            if ctx.Terminals.has_key(p.prod[0]):
...
def lalr_parse_table():
    ...
    EmptyAncestors = {}
    for y in ctx.Prodempty.keys():
        EmptyAncestors[y] = []
    for x in ctx.NTReductions.items():
        for y in x[1]:
            if ctx.Prodempty.has_key(y):
            ...
            try:
                if p.prod[-1] == ".":
                        ...
                        for i in ancestors:
                            for s in K:
                                if i in s:
                                    ...
                                    #print "ctx.Follow[p.name]: %s" % ctx.Follow[p.name]
                                    for a in input_list:
                                        actlist.append((a,p,"reduce using rule %d (%s) " % (p.number,p)))
                                        r = action.get((st,a),None)
                                        if r is not None:
                    ...
                    if ctx.Terminals.has_key(a):
                        ...
                        nonterminal = a
                        term_list = ctx.TReductions[nonterminal]
...
_lr_signature = %s
""" % (filename, repr(_lr_method), repr(ctx.Signature.digest())))
...
def lr_read_tables(module=tab_module,optimize=0):
    ...
    try:
        ...
        if (optimize) or (ctx.Signature.digest() == parsetab._lr_signature):
...
def yacc(method=default_lr, debug=yaccdebug, module=None, tabmodule=tab_module, start=None, check_recursion=1, optimize=0,write_tables=1,debugfile=debug_file,outputdir=''):
    ...
    if start:
        ctx.Signature.update(start)

    # Add parsing method to signature
    ctx.Signature.update(method)
    ...
    if (optimize and lr_read_tables(tabmodule,1)):
        ...
        if prec:
            ...
            add_precedence(prec)
            ctx.Signature.update(repr(prec))

        for n in tokens:
            if not ctx.Precedence.has_key(n):
                ctx.Precedence[n] = ('right',0)         # Default, right associative, 0 precedence
        ...
        for f in symbols:
            if f.__doc__:
                ctx.Signature.update(f.__doc__)
        ...
        if not lr_read_tables(tabmodule):
            ...
            if start and not ctx.Prodnames.has_key(start):
    ...
    p = Parser("xyzzy")
    p.productions = ctx.Productions
    p.errorfunc = ctx.Errorfunc
    p.action = _lr_action
    p.goto   = _lr_goto
    p.method = _lr_method
    p.require = ctx.Requires
...
def yacc_cleanup():
    ...
    del ctx.Productions, ctx.Prodnames, ctx.Prodmap, ctx.Terminals
    del ctx.Nonterminals, ctx.First, ctx.Follow, ctx.Precedence, ctx.LRitems
    del ctx.Errorfunc, ctx.Signature, ctx.Requires
    del ctx.Prodempty, ctx.TReductions, ctx.NTReductions, ctx.GotoSetNum, ctx.Canonical
